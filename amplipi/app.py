...
def announce(announcement: models.Announcement, ctrl: Api = Depends(get_ctrl)) -> models.Status:
  """ Announcements are not supported in this build — use Lyrion TTS instead. """
  return code_response(ctrl, ApiResponse.error('announce not supported: use Lyrion TTS'))


@api.post('/api/play', tags=['play'])
def play_media(media: models.PlayMedia, ctrl: Api = Depends(get_ctrl)) -> models.Status:
  """ Play media — not supported in this build. Use Lyrion instead. """
  return code_response(ctrl, ApiResponse.error('play not supported: use Lyrion'))
